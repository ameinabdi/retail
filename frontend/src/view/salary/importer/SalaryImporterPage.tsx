import React from 'react';
import { i18n } from 'src/i18n';
import actions from 'src/modules/salary/importer/salaryImporterActions';
import fields from 'src/modules/salary/importer/salaryImporterFields';
import selectors from 'src/modules/salary/importer/salaryImporterSelectors';
import ContentWrapper from 'src/view/layout/styles/ContentWrapper';
import Breadcrumb from 'src/view/shared/Breadcrumb';
import importerHoc from 'src/view/shared/importer/Importer';
import PageTitle from 'src/view/shared/styles/PageTitle';

const SalaryImportPage = (props) => {
  const Importer = importerHoc(
    selectors,
    actions,
    fields,
    i18n('entities.salary.importer.hint'),
  );
  return (
    <>
      <Breadcrumb
        items={[
          [i18n('dashboard.menu'), '/'],
          [i18n('entities.salary.menu'), '/salary'],
          [i18n('entities.salary.importer.title')],
        ]}
      />

      <ContentWrapper>
        <PageTitle>
          {i18n('entities.salary.importer.title')}
        </PageTitle>

        <Importer />
      </ContentWrapper>
    </>
  );
};

export default SalaryImportPage;
