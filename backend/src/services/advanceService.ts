...
import AdvanceRepository from '../database/repositories/advanceRepository';
...
export default class AdvanceService {
  ...
  async create(data) {
    ...
    try {
      ...
      const record = await AdvanceRepository.create(data, {
        ...this.options,
        transaction,
      });
  ...
  async update(id, data) {
    ...
    try {
      ...
      const record = await AdvanceRepository.update(
        id,
        data,
        {
          ...this.options,
  ...
  async destroyAll(ids) {
    ...
    try {
      for (const id of ids) {
        await AdvanceRepository.destroy(id, {
          ...this.options,
          transaction,
        });
      }
  ...
  async findById(id) {
    return AdvanceRepository.findById(id, this.options);
  }

  async findAllAutocomplete(search, limit) {
    return AdvanceRepository.findAllAutocomplete(
      search,
      limit,
      this.options,
    );
  ...
  async findAndCountAll(args) {
    return AdvanceRepository.findAndCountAll(
      args,
      this.options,
    );
  }
  ...
  async _isImportHashExistent(importHash) {
    const count = await AdvanceRepository.count(
      {
        importHash,
      },
      this.options,
