...
class ProductRepository {
  ...
  static async findAndCountAllStock(
      ...
      if (filter.productSerialNumber) {
        whereAnd.push(
          SequelizeFilterUtils.ilikeIncludes(
            'product',
            'productSerialNumber',
            filter.productSerialNumber,
          ),
        );
      }
      if (filter.status) {
        if(filter.status == 'stock'){
          whereAnd.push({
            ['totalAvailable']:{
              [Op.gt]: 0
            }
    ...
    let {
      rows,
      count,
    } = await options.database.stock.findAndCountAll({
      where,
      include,
  ...
  static async findAllSellAutocomplete(query, limit, options: IRepositoryOptions) {
    const tenant = SequelizeRepository.getCurrentTenant(
      options,
    );
  ...
  static async _fillWithRelationsAndFiles(record, options: IRepositoryOptions) {
    ...
    output.productPhoto = await FileRepository.fillDownloadUrl(
      await record.getProductPhoto({
        transaction,
      }),
    );
